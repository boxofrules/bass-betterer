...
// pitch shifter. Ship parameters: grain 32 ms, Hann crossfade,
// correlation-splice snap 32 ms. The ratio is fixed at prepare():
//   * ratio 2.0 = octave UP (the premium octave layers; 6 kHz pre-LPF on) —
//     unchanged from the original, byte-identical when configured up;
//   * ratio 0.5 = octave DOWN (GUITAR mode: play a guitar, get a bass;
//     pre-LPF off — down-shift creates no aliasing). Down is causal (the
//     tap delay only grows), so there is no lookahead.
//
// Architecture:
//   * ring buffer; TWO read taps whose delay sweeps at rate (1 - ratio):
//     UP sweeps grain -> 0 (read head races ahead at 2x, pitch doubles);
//     DOWN sweeps DMIN -> grain+DMIN (read head trails at 0.5x, pitch halves;
//     DMIN is a 2-sample floor so interpolation never reads past the write
//     head at the last sample of a block);
//   * each tap's delay wraps every W = grain/|1-ratio|; taps run half a
//     period apart;
...
//   * optional 2nd-order Butterworth pre-LPF before the shift (for UP:
//     content above the cutoff lands at cutoff*2 — anti-alias / de-fizz
//     insurance; unnecessary for DOWN and disabled there).
//
...
struct OctaveShifter
{
    ...
    // configs vs the real octave captures; REPORT.md is the record)
    static constexpr double GRAIN_MS   = 32.0;
    static constexpr double PRE_LPF_HZ = 6000.0;
    static constexpr double SNAP_MS    = 32.0;
    static constexpr double DMIN       = 2.0;      // down-shift min delay (samples), see header
    ...
    void prepare (double sampleRate, double ratio_, bool preLpfOn_)
    {
        sr    = sampleRate;
        ratio = ratio_;
        preLpfOn = preLpfOn_;
        ...
        W     = grain / std::max (std::abs (1.0 - ratio), 1.0e-6); // wrap period (grain at 2.0, 2*grain at 0.5)

        int n = 1;
        while (n < (int) grain + snap + tmpl + 8192 + 8)
            n <<= 1;
    ...
    // Latency reported to the host while the shifter is engaged.
    ...
    // = 32 ms at ship params (1536 smp @ 48 kHz; +DMIN when shifting down).
    // REPORT.md quotes the full worst-case range (~grain/2..grain+snap
    // ≈ 16-64 ms) for honesty.
    int latencySamples() const
    {
        return (int) std::lround (0.5 * (grain + (double) snap)
                                  + (ratio < 1.0 ? DMIN : 0.0));
    }
...
private:
    ...
    void processChunk (const double* in, double* out, int n)
    {
        ...
        // The LPF is an anti-alias guard for UP only; DOWN writes dry.
        if (preLpfOn)
            for (int i = 0; i < n; ++i)
            {
                const double x = in[i];
            ...
            for (int i = 0; i < n; ++i)
                buf[(size_t) ((wpos + (int64_t) i) & mask)] = in[i];
    ...
    // up-shift sweeps grain -> 0 over one wrap period; down-shift sweeps
    // DMIN -> grain+DMIN (delay only grows: causal, and the floor keeps the
    // interpolator behind the write head)
    double delayAt (int tap, double t) const
    {
        const double frac = (nextWrap[(size_t) tap] - t) / W;   // 1 at wrap -> 0 at next
        return ratio > 1.0 ? frac * grain
                           : (1.0 - frac) * grain + DMIN;
    }
    ...
    void splice (int tap, double tE)
    {
        ...
        // nominal post-wrap delay: grain for up, DMIN for down (the tap
        // restarts at its sweep origin)
        const int64_t posN = (int64_t) std::nearbyint (
            ratio > 1.0 ? tE - grain : tE - DMIN);
