...
**Does it phone home?** Two things, both harmless and both visible in the source (`Source/Analytics.h`). (1) The editor asks the public GitHub releases feed — at most once a day — whether a newer version exists, and shows an UPDATE notice if so. (2) It records **anonymous usage events** (that the plugin launched, its version/OS/host, which presets and major controls get used, and clicks on the footer links) so we know what to keep building in the free plugin. Events queue in a local file and are sent to boxofrules.com in a batch about once an hour — nothing is sent per click, and offline use just queues until later. Anonymous means anonymous: no personal data, no audio, no file or session names (your own preset names are never sent), and the server stores no IP addresses — the only identifier is a random ID the plugin invents on first run that maps to nothing. There's a visible opt-out in the SYS panel (**USAGE STATS**, next to the **PRIVACY** link) that stops all of it immediately; the full policy is at [boxofrules.com/privacy](https://boxofrules.com/privacy).
